...
class DDPM(pl.LightningModule):
    ...
    def get_input(self, batch):
        ...
        pose = pose.to(memory_format=torch.contiguous_format).float()
        ref_pose = ref_pose.to(memory_format=torch.contiguous_format).float()
        real_mask = real_mask.to(memory_format=torch.contiguous_format).float()

        return x, inpaint, mask, reference, hint, real_mask, pose, ref_pose
